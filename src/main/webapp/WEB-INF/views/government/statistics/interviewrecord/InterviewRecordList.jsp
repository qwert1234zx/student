...
<body>
	...
	function search(newPageNum) {
		$.ajax({
			type: "post",
			url: "<%=request.getContextPath()%>/government/statistics/interviewrecord/search",
	...
	/*添加*/
	function add(){
        var url = "<%=request.getContextPath()%>/government/statistics/interviewrecord/addForward";		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '添加',						//弹出页面标题
            type: 2,							//2--解析URL
	...
	/*编辑*/
    function edit(id){
        var url = "<%=request.getContextPath()%>/government/statistics/interviewrecord/updateForward?testTableId="+id;		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '修改',						//弹出页面标题
            type: 2,							//2--解析URL
            area: ['400', '450'],				//弹出层大小，也可用%来控制页面比例
		...
		if( sCheckboxList.length == 0 ){
            ...
            layer.confirm('所有选择的数据都将被删除，您确定吗？',function () {
                $.ajax({
                    ...
                    type: "post",
                    url: "<%=request.getContextPath()%>/government/statistics/interviewrecord/deleteByIdList",
					dataType: "json",
                    success:function (result) {
                        if (result > 0) {
