...
<body>
	<nav class="breadcrumb"><i class="Hui-iconfont"></i>首页 <span class="c-999 en">&gt;</span> 系统管理 <span class="c-666 en">&gt;</span><span class="c-666">专业</span> <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="Hui-article">
		<article class="cl pd-20" style="padding-left: 200px">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				...
				<a id="add" href="javascript:;" onclick="add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加专业</a>
				
	<input type="text" id="Description" placeholder="专业简介" style="width:250px" class="input-text">
	<input type="text" id="Degree" placeholder="学历" style="width:250px" class="input-text">
	<input type="text" id="SpecialityName" placeholder="专业名称" style="width:250px" class="input-text">
	...
	<td width="15%">所属学校</td>
				...
				<tbody id="listtable">
					<c:forEach items="${DataList}" var="Specialty" varStatus="status">
						<tr class='text-c'>
							<td><input width='25' type='checkbox' value='${Specialty.specialtyID}' name='get_id'/></td>
							<td>${status.index+1}</td>
							
	<td>${Specialty.description}</td>
	<td>${Specialty.degree}</td>
	<td>${Specialty.specialityName}</td>
	<td>${Specialty.academyID}</td>
	<td>${Specialty.universityID}</td>

							<td>
								<a title='编辑' onclick='edit(${Specialty.specialtyID})' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>
								<a title='删除' onclick='deleteById(${Specialty.specialtyID})' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a>
								<a title='详情' onclick='showDetail(${Specialty.specialtyID})' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe665;</i></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	function search(newPageNum) {
		$.ajax({
			type: "post",
			url: "<%=request.getContextPath()%>/university/specialitymanagement/specialty/search",
			data: {"pageNum": newPageNum,  "description":Description.value, "degree":Degree.value, "specialityName":SpecialityName.value, "academyID":AcademyID.value, "universityID":UniversityID.value},	//查询参数可以根据需要来定制，参数名称注意要与Controller中方法参数名称一致，查询后跳转到当前所在页以支持AJAX下的翻页
	...
	//            $("#listtable").empty();	//DHTML的bug？使用empty()会始终保留一个空行导致用append方法最后页面格式不对
				...
				for (var i = 0; i < data.length; i++) {
					...
					generatedHTML+="<td>" + data[i].specialityName + "</td>";
					...
					if (parseInt(functionAuthorityString, 2) & parseInt(10, 2)) {
						var strupdate = "<a title='编辑' onclick='edit(" + data[i].specialtyID + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
						generatedHTML+=strupdate;
					}
					if (parseInt(functionAuthorityString, 2) & parseInt(1000, 2)) {
						var strdelete = "<a title='删除' onclick='deleteById(" + data[i].specialtyID + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
						generatedHTML+=strdelete;
					}
					if (parseInt(functionAuthorityString, 2) & parseInt(100, 2)) {
						var strdetail = "<a title='详情' onclick='showDetail(" + data[i].specialtyID + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe665;</i></a>";
	...
	/*添加*/
	function add(){
        var url = "<%=request.getContextPath()%>/university/specialitymanagement/specialty/addForward";		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '添加',						//弹出页面标题
            type: 2,							//2--解析URL
	...
	/*编辑*/
    function edit(id){
        var url = "<%=request.getContextPath()%>/university/specialitymanagement/specialty/updateForward?testTableId="+id;		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '修改',						//弹出页面标题
            type: 2,							//2--解析URL
            area: ['400', '450'],				//弹出层大小，也可用%来控制页面比例
		...
		if( sCheckboxList.length == 0 ){
            ...
            layer.confirm('所有选择的数据都将被删除，您确定吗？',function () {
                $.ajax({
                    ...
                    type: "post",
                    url: "<%=request.getContextPath()%>/university/specialitymanagement/specialty/deleteByIdList",
					dataType: "json",
                    success:function (result) {
                        if (result > 0) {
