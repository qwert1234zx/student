...
<body>
	...
	是否双一流:
	<select id="IsDFG" size="1">
		<option value="">请选择</option>
		<option   value="1">是</option>
		<option   value="0">否</option>
	</select>
	是否211:
	<select id="IsTOO" size="1">
		<option value="">请选择</option>
		<option   value="1">是</option>
		<option   value="0">否</option>
	</select>
    ...
    <input type="text" id="ProvinceName" placeholder="所属省" style="width:250px" class="input-text">
	...
	<td>${University.degreeName}</td>
	<td>
	<c:choose>
		<c:when test="${University.isDFG==1}">
			是
		</c:when>
		<c:when test="${University.isDFG==0}">
			否
		</c:when>
	</c:choose>
	</td>
	<%--<td>${University.isDFG}</td>--%>
							<td>
								<c:choose>
									<c:when test="${University.isTOO==1}">
										是
									</c:when>
									<c:when test="${University.isTOO==0}">
										否
									</c:when>
								</c:choose>
							</td>
	<%--<td>${University.isTOO}</td>--%>
							<td>
								<c:choose>
									<c:when test="${University.isNEF==1}">
										是
									</c:when>
									<c:when test="${University.isNEF==0}">
										否
									</c:when>
								</c:choose>
							</td>
	...
	function search(newPageNum) {
		$.ajax({
			type: "post",
			url: "<%=request.getContextPath()%>/university/infomationmanagement/university/search",
			data: {"pageNum": newPageNum,  "degree":Degree.value, "isDFG":IsDFG.value, "isTOO":IsTOO.value, "isNEF":IsNEF.value, "description":Description.value, "universityName":UniversityName.value, "provinceName":ProvinceName.value},	//查询参数可以根据需要来定制，参数名称注意要与Controller中方法参数名称一致，查询后跳转到当前所在页以支持AJAX下的翻页
...
//                for (var i = 0; i < data.length; i++) {
					...
					generatedHTML+="<td><input width='25' type='checkbox' value=" + data[i].universityID + " name='get_id'/></td>";
					...
					generatedHTML+="<td>" + data[i].degreeName + "</td>";
					generatedHTML+="<td>" + x + "</td>";
					generatedHTML+="<td>" + y + "</td>";
					generatedHTML+="<td>" + z + "</td>";
...
//                parent.shua();
	...
	/*添加*/
	function add(){
        var url = "<%=request.getContextPath()%>/university/infomationmanagement/university/addForward1";		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '添加',						//弹出页面标题
            type: 2,							//2--解析URL
	...
	/*编辑*/
    function edit(id){
        var url = "<%=request.getContextPath()%>/university/infomationmanagement/university/updateForward?testTableId="+id;		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '修改',						//弹出页面标题
            type: 2,							//2--解析URL
            area: ['400', '450'],				//弹出层大小，也可用%来控制页面比例
		...
		if( sCheckboxList.length == 0 ){
            ...
            layer.confirm('所有选择的数据都将被删除，您确定吗？',function () {
                $.ajax({
                    ...
                    type: "post",
                    url: "<%=request.getContextPath()%>/university/infomationmanagement/university/deleteByIdList",
					dataType: "json",
                    success:function (result) {
                        if (result > 0) {
