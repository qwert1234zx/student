...
<body>
	...
	<div class="Hui-article">
		<article class="cl pd-20" style="padding-left: 200px">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				...
				<a id="add1" href="javascript:;" onclick="add1()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加学生查询条件</a>
	<input type="text" id="QueryName" placeholder="学生查询条件名称" style="width:250px" class="input-text">
	<input type="text" hidden="hidden" id="StudentID" placeholder="所属学生" style="width:250px" class="input-text">
	...
	<td width="15%">学生查询条件名称</td>
				...
				<tbody id="listtable">
					<c:forEach items="${DataList}" var="StudentQueryCondition" varStatus="status">
						<tr class='text-c'>
							<td><input width='25' type='checkbox' value='${StudentQueryCondition.studentQueryCondition}' name='get_id'/></td>
							<td>${status.index+1}</td>
							
	<td>${StudentQueryCondition.queryName}</td>
	<%--<td>${StudentQueryCondition.studentID}</td>--%>

							<td>
								<a href="<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/watchone?id=${StudentQueryCondition.studentQueryCondition}">查看</a>
								<a title='编辑' onclick='edit(${StudentQueryCondition.studentQueryCondition})' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>
								<a title='删除' onclick='deleteById(${StudentQueryCondition.studentQueryCondition})' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a>
								<a href="<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/result?id=${StudentQueryCondition.studentQueryCondition}">执行</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
	...
	//            $("#listtable").empty();	//DHTML的bug？使用empty()会始终保留一个空行导致用append方法最后页面格式不对
				...
				for (var i = 0; i < data.length; i++) {
					...
					generatedHTML+="<td><input width='25' type='checkbox' value=" + data[i].studentQueryCondition + " name='get_id'/></td>";
...
//					generatedHTML+="<td>" + data[i].studentID + "</td>";
					...
					generatedHTML+="<a href='<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/watchone?id="+data[i].studentQueryCondition+"'>查看</a>";
					if (parseInt(functionAuthorityString, 2) & parseInt(10, 2)) {
						var strupdate = "<a title='编辑' onclick='edit(" + data[i].studentQueryCondition + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
						generatedHTML+=strupdate;
					}
					if (parseInt(functionAuthorityString, 2) & parseInt(1000, 2)) {
						var strdelete = "<a title='删除' onclick='deleteById(" + data[i].studentQueryCondition + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
						generatedHTML+=strdelete;
					}
//					if (parseInt(functionAuthorityString, 2) & parseInt(100, 2)) {
//						var strdetail = "<a title='详情' onclick='showDetail(" + data[i].studentQueryCondition + ")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe665;</i></a>";
//						generatedHTML+=strdetail;
//					}
                    generatedHTML+="<a href='<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/result?id="+data[i].studentQueryCondition+"'>执行</a>";
					generatedHTML+="</td>";
					generatedHTML+="</tr>";
				}
	...
	/*编辑*/
    function edit(id){
        var url = "<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/watch?id="+id;		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '修改',						//弹出页面标题
            type: 2,							//2--解析URL
            area: ['50%', '60%'],				//弹出层大小，也可用%来控制页面比例
            ...
            if (sCheckboxList.length == 0) {
                ...
                layer.confirm('所有选择的数据都将被删除，您确定吗？', function () {
                    $.ajax({
                        ...
                        type: "post",
                        url: "<%=request.getContextPath()%>/student/jobsearch/studentquerycondition/deleteByIdList1",
                        dataType: "json",
                        success: function (result) {
                            if (result > 0) {
