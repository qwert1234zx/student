...
<body>
	...
	<input type="text" id="StudentRemark" placeholder="学生说明" style="width:250px" class="input-text">
	<input type="text" id="StudentRate" placeholder="学生评价" style="width:250px" class="input-text">
	<input type="text" id="InterviewerRemark" placeholder="面试官评价" style="width:250px" class="input-text">
	<input type="text" id="InterviewerConclusion" placeholder="面试官结论" style="width:250px" class="input-text">
	...
	<input type="text" id="CandidateID" placeholder="候选人ID" style="width:250px" class="input-text">
	...
	<td width="15%">学生说明</td>
	<td width="15%">学生评价</td>
	<td width="15%">面试官评价</td>
	<td width="15%">面试官结论</td>
	<td width="15%">面试官手机</td>
	<td width="15%">面试官</td>
	<td width="15%">面试时间</td>
	<td width="15%">候选人ID</td>
	...
	<td>${InterviewRecord.studentRemark}</td>
	<td>${InterviewRecord.studentRate}</td>
	<td>${InterviewRecord.interviewerRemark}</td>
	<td>${InterviewRecord.interviewerConclusion}</td>
	<td>${InterviewRecord.interviewerMobile}</td>
	<td>${InterviewRecord.interviewer}</td>
	<td>${InterviewRecord.interviewTime}</td>
	<td>${InterviewRecord.candidateID}</td>
	...
	function search(newPageNum) {
		$.ajax({
			type: "post",
			url: "<%=request.getContextPath()%>/enterprise/interviewmanagement/interviewrecord/search",
			data: {"pageNum": newPageNum,  "studentRemark":StudentRemark.value, "studentRate":StudentRate.value, "interviewerRemark":InterviewerRemark.value, "interviewerConclusion":InterviewerConclusion.value, "interviewerMobile":InterviewerMobile.value, "interviewer":Interviewer.value, "interviewTime":InterviewTime.value, "candidateID":CandidateID.value},	//查询参数可以根据需要来定制，参数名称注意要与Controller中方法参数名称一致，查询后跳转到当前所在页以支持AJAX下的翻页
	...
	//            $("#listtable").empty();	//DHTML的bug？使用empty()会始终保留一个空行导致用append方法最后页面格式不对
				...
				for (var i = 0; i < data.length; i++) {
					...
					generatedHTML+="<td>" + data[i].studentRemark + "</td>";
					generatedHTML+="<td>" + data[i].studentRate + "</td>";
					generatedHTML+="<td>" + data[i].interviewerRemark + "</td>";
					generatedHTML+="<td>" + data[i].interviewerConclusion + "</td>";
					...
					generatedHTML+="<td>" + data[i].candidateID + "</td>";
	...
	/*添加*/
	function add(){
        var url = "<%=request.getContextPath()%>/enterprise/interviewmanagement/interviewrecord/addForward";		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '添加',						//弹出页面标题
            type: 2,							//2--解析URL
	...
	/*编辑*/
    function edit(id){
        var url = "<%=request.getContextPath()%>/enterprise/interviewmanagement/interviewrecord/updateForward?testTableId="+id;		//要弹出的路径 PS：其实就是正常访问的页面
        layer.open({
            title: '修改',						//弹出页面标题
            type: 2,							//2--解析URL
            area: ['400', '450'],				//弹出层大小，也可用%来控制页面比例
		...
		if( sCheckboxList.length == 0 ){
            ...
            layer.confirm('所有选择的数据都将被删除，您确定吗？',function () {
                $.ajax({
                    ...
                    type: "post",
                    url: "<%=request.getContextPath()%>/enterprise/interviewmanagement/interviewrecord/deleteByIdList",
					dataType: "json",
                    success:function (result) {
                        if (result > 0) {
