...
<head>
	...
	<title>修改具体所选专业条件</title>
</head>
<body>
<article class="cl pd-20" class="pos-ab">
	<div class="form form-horizontal" >
		<input type="hidden" class="input-text" id="SelectedSpecialityConditionId" maxlength="10" style="width:120px" value="${SelectedSpecialityCondition.selectedSpecialityConditionID}">
		
		<div class='row cl'>
			<label class='form-label col-xs-4 col-sm-2'><span class='c-red'>*</span>选中专业：</label>
			<div class='formControls col-xs-8 col-sm-9'><input type='text' class='input-text' id='SelectedSpecialityID' maxlength='10' style='width:120px' value='${SelectedSpecialityCondition.selectedSpecialityID}'></div>
		</div>
		<div class='row cl'>
			<label class='form-label col-xs-4 col-sm-2'><span class='c-red'>*</span>所属企业查询条件：</label>
			<div class='formControls col-xs-8 col-sm-9'><input type='text' class='input-text' id='EnterpriseQueryConditionID' maxlength='10' style='width:120px' value='${SelectedSpecialityCondition.enterpriseQueryConditionID}'></div>
		</div>
...
<script type="text/javascript">
    ...
    function submit() {
        if (validate(regList)) {
            //从各字段取数据拼接Json字符串
            var oSelectedSpecialityConditionId= SelectedSpecialityConditionId.value;
			
			var oSelectedSpecialityID = $('#SelectedSpecialityID').val();
			var oEnterpriseQueryConditionID = $('#EnterpriseQueryConditionID').val();

            var entity = {
                SelectedSpecialityConditionId:oSelectedSpecialityConditionId,
            ...
            $.ajax({
                type: "post",
                url: "<%=request.getContextPath()%>/enterprise/studentsearch/selectedspecialitycondition/update",
